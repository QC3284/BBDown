...
// → 「找不到 ffmpeg」用例变红。
func doctorWith(t *testing.T, nav string, status int) (string, int) {
	t.Helper()
...
func TestDoctorFailsOnRiskControl(t *testing.T) {
	out, code := doctorWith(t, "{}", http.StatusPreconditionFailed)
...
func TestDoctorFailsWhenFFmpegMissing(t *testing.T) {
	orig := muxerFFmpegForTest(t)
	_ = orig
	// 用一个必然不存在的可执行文件名，避免依赖机器上是否装了 ffmpeg。
