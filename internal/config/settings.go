...
// HDRVividID 是最高档 HDR Vivid 的清晰度 ID（129；上游 v1.6.20 没有这一档，来自 C# 2.x 接手线 BBDownT）。
// 抽成常量是因为它有两处用途：映射表与「HDR 兼容性提醒」——各处写死字面量时，档位一变就会静默失效。
const HDRVividID = "129"

// QualityMap maps Bilibili quality IDs to display names.
var QualityMap = map[string]string{
	HDRVividID: "HDR Vivid", // 上游 v1.6.20 没有这一档（C# 2.x 接手线 BBDownT 新增）；APP 协议最高清晰度相应提到 129
	"127":      "8K 超高清",
	"126":      "杜比视界",
	"125":      "HDR 真彩",
	"120":      "4K 超清",
