...
func TestMultiThreadDownloadFinishesProgressLineBeforeLogging(t *testing.T) {
	...
	renderProgressBar = func(_ *atomic.Int64, _ int64, _ progressPacer, done <-chan struct{}, stopped chan<- struct{}, _ func(ProgressEvent)) {
		<-done
...
func TestAggregateProgressClearsLineOnFinish(t *testing.T) {
	...
	out := captureStdout(t, func() {
		go renderAggregateProgress(&counter, 100, newProgressPacer(), done, stopped, nil)
		close(done)
		<-stopped
	})
