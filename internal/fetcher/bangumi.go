...
// BangumiInfoFetcher fetches bangumi/anime info.
...
func (f *BangumiInfoFetcher) Fetch(ctx context.Context, id string) (*entity.VInfo, error) {
	...
	api := fmt.Sprintf("https://api.bilibili.com/pgc/view/web/season?ep_id=%s", epID)
	...
	if result.Code != 0 {
		return nil, fmt.Errorf("获取番剧信息失败 (code=%d): %s", result.Code, result.Message)
	}
