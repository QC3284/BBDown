...
**CLI 视觉重做**（用户两次反馈「不算好看」后，从「微调排版」改为「给输出一套设计」）。

### 根因

我此前复核输出**一直在管道里看**——而管道按设计是无色的，等于**从没看过它在终端里的样子**却在改它的美观。用 `script` 抓伪终端后真相是：TTY 下只有 5 种 ANSI 码——
`[44m[97m` **蓝底白字横幅** + `[96m` 亮青段标题 + `[90m` 灰表头 + 复位，其余全是**同权重白字**。问题不是排版细节，是**没有视觉设计**。

### 新设计（4 个颜色角色 + 明度层级）

- **BRAND**（`38;5;45`，16 色回退 `96`）：段竖条 `▎`、选中标记 `›`、进度填充、横幅名称；
- **MUTED**（`38;5;245` / `90`）：时间戳、标签、表头、计数、单位、统计——**次要信息一律降明度**；
- **TEXT**：正文与数据行（不加色）；**BOLD**：关键值（时长、BV、百分比、汇总文案）；
- **状态色**：成功 `78`/警告 `214`/错误 `203`；**零背景色**（蓝底/红底横幅与失败块全部移除）。
- 结构：段标题改 1 字符竖条（长横线全删）；进度条 `█/░` 20 格、百分比 `%6.1f` 固定列（**0→100% 帧宽恒定**）；选中行 `›` + 整行加粗；
  横幅改 `❯ BBDown  2.12.6`（无底色）；失败块 `✗ 原因` + 标签降明度；汇总行 `✓ 下载完成   成功 1 · 失败 0 · 1.8s`。
- **降级链**：非 TTY / `NO_COLOR` / `TERM=dumb` → **一个 ANSI 都不发且版式一字不变**；256 色不可用 → 16 色回退（判定可注入）。实测 ESC 字节：256 TTY 64 / 16 TTY 64 / `NO_COLOR` 0 / `TERM=dumb` 0 / 管道 0。
...
`-I` 非 TTY 裸 URL、`--print-urls`、`--info-json`、`--progress-json` 逐字节不变（守卫用例一字未改且全绿；「降级链」变异会把 `-I` 管道契约打红，说明两层是同一条防线）。

### 说明

- 版本位：视觉/结构改动、无新能力 → **patch**。
- 登记台账 §4.61；保留的有意判断：进度条**保留动画字符**（静默心跳重画需要生命体征）、段标题不加可选横线、doctor 走不带状态标记的事件入口（避免与自带状态列双重标注）。
## [2.12.5] - 2026-09-26
