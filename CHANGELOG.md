...
**修两条可用性问题**（用户报告；观感保持上游，未动配色与主题）：流列表太宽/无列标签/不自适应；进度条撑到换行。

### 修复

- **流列表自适应 + 列标签**：列宽此前六列写死（72 列）+ 内容行 28 列缩进 = 整行 **100 列**，80 列终端必然折行，而且**没有表头**（用户不知道每列是什么）。
  现在列宽由 `util.TerminalWidth()` 每次渲染算一次（**表头与数据行共用同一套排版来源**），每段列表打一行表头；
  放不下时按 **体积 → 帧率 → 编码 → 分辨率** 省列 → 缩列宽 → 最后悬挂折行（续行对齐数据列，不顶格）；
  缩进是装饰、数据列不是：宽度不足时回收缩进，保证至少留下「清晰度 + 分辨率 + 编码 + 码率」四列（≥82 列时缩进仍是 28，外观不变）。
- **`-I` 直链**：**TTY 下**显示为 `↳ host/…/文件名`（1 行），**管道/重定向下逐字节保持完整裸 URL**（契约用例继续钉住）。
- **进度条不再撑到换行**：满字段帧实测 **116 列**（28 缩进 + 40 格条 + 动画 + 44 列统计区，此前我以为的 88 列是漏算了缩进）。
  现在 bar 格数由「终端宽度 − 缩进 − 固定开销 − 统计区」**反算**（上限 40、下限 10），不足时按 **ETA → 总量 → 速率** 裁统计区，最后才回收缩进。

### 数字（真机 pty，`script -qec` + `stty cols`，逐行按东亚宽度量）

| 场景 | 改前 | 改后 |
|---|---|---|
| `-I` 清单行（80 列终端） | 100 列 → 折行 | **≤80 列，0 行超宽**（4 条表头 + 数据行 + 直链提示行） |
| `-I` 直链 | 每条 600+ 字符裸 URL | TTY 单行 `↳ …`；管道仍逐字节原样 |
| 下载进度帧（80 列，单线程 29 帧） | 116 列 → 换行残帧 | **全部恰好 80 列，超宽 0** |
| 下载进度帧（120 列） | — | 91–92 列（≤120），全字段含 ETA |

### 说明

- 版本位：可用性修复 → **patch**。
- **事件行不参与宽度契约**（带时间戳前缀的提示/URL 行仍可能超过 80 列，与上游一致）：改它等于改整个 CLI 的日志外观，而用户刚要求保持上游观感。
- 一处刻意的微调：帧率列宽 3 → 6（实测 playurl 的 fps 是 `30.000` 六列，3 列时帧率之后的编码/码率/体积整列右移，加了表头会明显错位）；只在 ≥110 列终端可见。
## [2.12.7] - 2026-09-26

**还原上游观感**：撤回 `2.12.5`（输出清爽化）与 `2.12.6`（视觉重做）的全部外观改动，回到与上游一致的呈现。

### 为什么撤回

用户明确要求「还原上游的」。这两轮我按自己的审美改了事件前缀、内容通道、段标题、表格宽度、颜色角色与降级链——
虽然保住了机器契约面，但**那是我们自己的风格，不是上游的**。用户要的是上游那套：
...
- 版本行（蓝底白字）+ 求助链接回到开场；
- 事件行回到完整时间戳 `[日期 时间.毫秒] - `；
- 流列表回到上游的方括号格式、进度条回到 40 格 `#`/`-`、`doctor` 回到 `[ok]/[warn]/[fail]`；
- 内容与事件不再分流（此前新增的内容通道与主题模块随提交一并移除）。

### 说明

- 版本位：外观回退 → **patch**（`2.12.4` → `2.12.7`，跳过已被撤回的两个号以免混淆）。
- 这两轮的设计与实现**保留在 git 历史**（`ef40461`、`9d36672`）与台账 §4.60/§4.61（随撤回一并移除，若要复用可 `git show` 取回）：
  将来若要做「可选主题」，正确做法是**加开关**（默认上游风格），而不是改默认观感。
- **不受影响的部分**（本轮完全未触碰）：`--nfo`、`--compat`、`--write-m3u`、`--info-json`、`--print-urls`、`--log-file`、`resume`、
  重试分类、412 提示、SSE/Web UI、进度观察者、节流修复、宽度表收敛、跨平台守卫等——它们与外观无关。
## [2.12.4] - 2026-09-26
