...
- **失败输出**：运行期失败只打印错误消息 +「请尝试升级到最新版本后重试!」，两行按上游白字红底；只有参数/用法错误才附帮助文本（§4.32.1、§4.33）。
- **412 追加可操作提示**：`HTTP 412` 会附一句「等几分钟或换网络出口」——上游是 .NET 默认异常消息（§4.33）。
- **镜像 404 回退原地址**：`--force-replace-host`/`--upos-host` 改写后首次 404 即换回替换前的地址，单线程与分片两条路径都覆盖（上游会对同一个地址重试满 3×3 次，§4.33）。
- **请求更省（结果等价）**：解析阶段改为「qn=127 优先、失败或不带 DASH 才回落 qn=0」（少发一次 playurl），`-I` 跳过章节抓取。实测单次解析 **5 → 3 个请求、0.53 s → 0.27~0.32 s**，输出逐行一致（§4.35）。
